...
import {
  ...
  ForgotPasswordVerifyOtpAdminResponseDto,
...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description:
      'Creates an Admin account and sends OTP to email for verification',
  })
  ...
  @ApiBody({ type: VerifyOtpAdminDto })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'OTP verified successfully. Registration completed.',
    schema: {
  ...
  async verifyRegistrationOtpAdmin(
    @Body() dto: VerifyOtpAdminDto,
    @Res({ passthrough: true }) res: Response,
    @Req() req: Request,
  ) {
    const result = await this.adminService.verifyOtpForRegistration(dto, req);
  ...
  /**
   * 3. Login Admin - Send OTP
   */
  @Post('admin/login')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Login Admin',
    description: 'Authenticates Admin and sends OTP for 2FA',
  })
  ...
  async loginAdmin(
    ...
    return this.adminService.loginAdmin(dto, req);
  }

  /**
   * 4. Login Admin - Verify OTP & Issue Tokens
   */
  ...
  async verifyLoginOtpAdmin(
    @Body() dto: VerifyOtpAdminDto,
    @Res({ passthrough: true }) res: Response,
    @Req() req: Request,
  ) {
    const result = await this.adminService.verifyOtpForLogin(dto, req);
  ...
  /**
   * 5. Forgot Password Admin - Send OTP
   */
  @Post('forgot-password/admin')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Initiate forgot password for Admin',
    description: 'Sends OTP to Admin email for password reset',
  })
  @ApiBody({ type: ForgotPasswordAdminDto })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'OTP sent to your email',
    type: ForgotPasswordAdminResponseDto,
  })
  @ApiResponse({
    status: HttpStatus.NOT_FOUND,
  ...
  async forgotPasswordAdmin(
    @Body() dto: ForgotPasswordAdminDto,
  ): Promise<ForgotPasswordAdminResponseDto> {
    return this.adminService.forgotPassword(dto);
  }

  /**
   * 6. Forgot Password Admin - Verify OTP
   */
  @Post('forgot-password-verify-otp/admin')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Verify OTP for forgot password',
    description: 'Verifies OTP and returns reset token',
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'OTP verified successfully',
    type: ForgotPasswordVerifyOtpAdminResponseDto,
  })
  @ApiResponse({
    status: HttpStatus.UNAUTHORIZED,
    description: 'Invalid or expired OTP',
  })
  async verifyForgotPasswordOtpAdmin(
    @Body() dto: VerifyOtpAdminDto,
    @Res({ passthrough: true }) res: Response,
  ): Promise<ForgotPasswordVerifyOtpAdminResponseDto> {
    const result = await this.adminService.verifyForgotPasswordOtp(dto);
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Password reset successfully',
    type: ResetPasswordAdminResponseDto,
  })
  @ApiResponse({
    status: HttpStatus.UNAUTHORIZED,
    description: 'Invalid or expired reset token',
  ...
  async resetPasswordAdmin(
    ...
    @Res({ passthrough: true }) res: Response,
  ): Promise<ResetPasswordAdminResponseDto> {
    const result = await this.adminService.resetPassword(dto);
  ...
  async getUserActivities(@Query() query: QueryUserActivityDto) {
